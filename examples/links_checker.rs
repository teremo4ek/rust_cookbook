...
fn control_crawl(
    ...
    while pending_urls > 0 {
        ...
        match crawl_result {
            ...
            Err((url, error)) => {
                println!("issue : url: {:#}, error: {:#}", url, error);
                bad_urls.push(url);
                continue;
            }
        }
...
fn check_links(start_url: Url) -> Vec<Url> {
    ...
    spawn_crawler_threads(command_receiver, result_sender, 1);
    control_crawl(start_url, command_sender, result_receiver)
}

fn main() {
    let start_url = reqwest::Url::parse("https://teremo4ek.github.io").unwrap();
    let bad_urls = check_links(start_url);
    println!("BAD URL: {:#?}", bad_urls);
}
